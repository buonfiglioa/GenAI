...
# Run a Python script
uv run python lmstudio_script.py/lmstudio_script.py

# Launch Jupyter for notebooks
uv run jupyter lab
```

The project requires CUDA-capable GPUs. Notebooks use `device_map="auto"` to spread models across all available GPUs. Typical memory requirements:
...
`data/` is gitignored; regenerate by running the notebooks in order.
...
- The LM Studio script uses `api_key="lm-studio"` (a dummy key) for the local OpenAI-compatible server.
