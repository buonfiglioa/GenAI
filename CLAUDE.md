...
- Part 3 GPU parallelism: when `torch.cuda.device_count() > 1` and the model is not in `_LARGE_MODELS`, one copy of the model is loaded per GPU and each mitigation runs on a separate GPU concurrently via `ThreadPoolExecutor` + a `queue.Queue` of GPU IDs. `google/gemma-3-12b-it` is in `_LARGE_MODELS` and always runs sequentially with `device_map="auto"` across all GPUs.
